...
export async function updatePerk(req, res, next) {
  try {
    ...
    const mergedData = { ...existingPerk.toObject(), ...req.body };
    const { value, error } = perkSchema.validate(mergedData, {
      stripUnknown: true,
    });
    if (error) return res.status(400).json({ message: error.message });
    Object.assign(existingPerk, req.body);
    const updatedPerk = await existingPerk.save();

    res.status(200).json({ message: "200 Updated", perk: updatedPerk });
  } catch (err) {
    if (err.code === 11000)
      return res
        .status(409)
